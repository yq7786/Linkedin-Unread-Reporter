...
4. Run `npm run scan` for a supervised dry scan. Explain before running it that opening an eligible unread thread may change LinkedIn's read state. A visible or hidden stable destination is validated and promoted to a direct-URL candidate, whose recovery marker is saved before opening; preview text is never used as a destination. For a truly anchorless candidate, it opens the exact row and handles visible blockers within the bounded authentication timeout. It accepts a canonical thread URL only when the same uniquely visible list identifies that row as its sole active row; `onOpened` then persists the marker before extraction or readiness. If responsive markup removes the list, or recovery finds the row read or missing, it fails closed with no marker. This truly anchorless path has a narrow unavoidable crash window between the row opening and marker persistence. The scan writes captured private data to the mode-`0600` `.linkedin-unread-outbox.json`, never calls the Portal, and prints counts only.
...
- Use the read-on-open boundary: open only an explicitly eligible unread one-to-one thread and capture only incoming messages within its pre-open unread boundary. Save a direct-URL candidate marker before opening. For a truly anchorless row, classify and recover visible blockers through the unread URL, then require the same uniquely visible list and clicked stable row identity as its sole active row before `onOpened` persists the marker. Fail closed with no marker when that correlation is unavailable; acknowledge the narrow unavoidable crash window before the callback persists. Checkpoint immediately after extraction by replacing the marker with extracted entries. Never open an ineligible row or use preview text as message content.
...
- Treat `.linkedin-unread-outbox.json` as a private durable outbox. It contains names, message content, thread URLs, and delivery state, must remain mode `0600`, and must never be read or summarized by Codex or a timestamp subagent. Direct-URL recovery, including promoted hidden stable destinations, is durable before opening; truly anchorless recovery becomes durable in `onOpened` immediately after URL validation and before thread readiness or extraction; completed extraction is then checkpointed immediately.
