...
test('skill metadata and instructions are complete and portable', async () => {
  ...
  assert.match(skill, /Legacy mode-less capture markers[^\n]+direct/i);
  assert.match(skill, /cannot be atomic across the two systems[^\n]+external-state race[^\n]+duplicate/i);
  assert.match(skill, /Portal idempotency keys mitigate duplicates/i);
  assert.match(skill, /promotion fails before commit[^\n]+preopen_pending/i);
  assert.match(skill, /failure or abort after rename[^\n]+direct intent[^\n]+next run[^\n]+at least once/i);
  assert.match(skill, /anchorless[^\n]+exact row[^\n]+active row[^\n]+onOpened[^\n]+before extraction/i);
  assert.match(skill, /responsive markup[^\n]+fails closed with no recovery marker/i);
